...
export class BasePage {
    ...
    async click(locator, elementName) {
        ...
        this.logger.info(`Finding locator: ${locatorInfo.selector} (${locatorInfo.name})`);
    ...
    async waitForElement(locator, elementName, timeout = 30000) {
        ...
        this.logger.info(`Waiting for ${locatorInfo.name} to appear: ${locatorInfo.selector}`);
    ...
    async assertElementVisible(locator, elementName) {
        ...
        this.logger.info(`Verifying ${locatorInfo.name} is visible: ${locatorInfo.selector}`);
    ...
    async assertElementContainsText(locator, expectedText, elementName) {
        ...
        this.logger.info(`Verifying ${locatorInfo.name} contains text: "${expectedText}"`);
        await expect(this.page.locator(locatorInfo.selector)).toContainText(expectedText);
        this.logger.info(`${locatorInfo.name} contains expected text: "${expectedText}"`);
    }
    ...
    async selectDropdown(locator, value, dropdownName) {
        const locatorInfo = this._getLocatorInfo(locator, dropdownName || 'dropdown');
        ...
        this.logger.info(`Successfully selected "${value}" from ${locatorInfo.name}`);
    }
}
