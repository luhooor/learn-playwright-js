...
export const test = base.extend({
    ...
    sheetsManager: async ({ }, use) => {
        ...
        try {
            ...
            console.warn('⚠️  Google Sheets not configured, using fallback test data');
    ...
    authenticatedPage: async ({ page, loginPage, sheetsManager }, use) => {
        ...
        if (sheetsManager) {
            try {
                userCredentials = await sheetsManager.getUserCredentials('standard_user');
            } catch (error) {
                console.warn('⚠️  Could not get user credentials from Google Sheets, using default');
    ...
    testData: async ({ sheetsManager }, use) => {
        let testData = {
            users: [
                { user_type: 'standard_user', username: 'standard_user', password: 'secret_sauce', description: 'Standard user' },
                { user_type: 'locked_out_user', username: 'locked_out_user', password: 'secret_sauce', description: 'Locked out user' },
                { user_type: 'problem_user', username: 'problem_user', password: 'secret_sauce', description: 'Problem user' },
                { user_type: 'performance_glitch_user', username: 'performance_glitch_user', password: 'secret_sauce', description: 'Performance user' }
            ],
            products: [
                // You'll add your own product data when you create product tests!
                { product_name: 'Sauce Labs Backpack', price: '$29.99', category: 'accessories' }
            ],
            customerInfo: {
                firstName: 'John',
                lastName: 'Doe',
        ...
        if (sheetsManager) {
            try {
                ...
                if (all.length > 0) {
                    ...
                    const usersFromSheet = all.filter(r => r.testdata && r.testdata.user_type).map(r => ({
                    ...
                    for (const r of all) {
                        if (r.testdata && (r.testdata.product_name || r.testdata.products)) {
                            ...
                            if (Array.isArray(r.testdata.products)) {
                                for (const p of r.testdata.products) productsFromSheet.push({ product_name: p });
                            }
                        }
                    }
                    if (productsFromSheet.length > 0) testData.products = productsFromSheet;

                    console.log('✅ Test data loaded from TestData sheet');
                } else {
                    // fallback to legacy Users sheet if present
                    const users = await sheetsManager.getTestData('Users!A1:D10').catch(() => testData.users);
                    testData.users = users.length > 0 ? users : testData.users;
                    console.log('✅ Test data loaded from Google Sheets (legacy)');
                }
            } catch (error) {
                console.warn('⚠️  Using fallback test data due to Google Sheets error:', error.message);
            }
        }
